...
if [ "$EUID" -ne 0 ]; then
    echo "Please run with sudo: sudo setup/setup.sh"
    exit 1
fi
...
Restart=always
RestartSec=10
StandardOutput=append:$PROJECT_DIR/worker.log
StandardError=append:$PROJECT_DIR/worker.log
...
echo "  Logs:   tail -f $PROJECT_DIR/worker.log"
...
chmod +x "$PROJECT_DIR/run.sh" "$PROJECT_DIR/cleanup.sh" 2>/dev/null
(sudo -u "$ACTUAL_USER" crontab -l 2>/dev/null | grep -v "auto_email_sender"; \
 echo "# Auto Email Sender - scrape LinkedIn + queue emails"; \
 echo "0 10 * * * $PROJECT_DIR/run.sh"; \
 echo "# Auto Email Sender - weekly log cleanup"; \
 echo "0 4 * * 0 $PROJECT_DIR/cleanup.sh"; \
 echo "# Auto Email Sender - Monday resend (re-queue last week's sent emails)"; \
 echo "0 10 * * 1 cd $PROJECT_DIR && /usr/bin/python3 $PROJECT_DIR/resend.py >> $PROJECT_DIR/logs/resend.log 2>&1") | sudo -u "$ACTUAL_USER" crontab -
