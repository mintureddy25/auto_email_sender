...
#
# Usage: ./setup/setup-mac.sh          (no sudo)
...
if [ "$(uname)" != "Darwin" ]; then
    echo "This script is macOS only. On Linux use: sudo setup/setup.sh"
    exit 1
fi
...
for key in RABBITMQ_URL EMAIL_USER EMAIL_PASSWORD EMAIL_SMTP_SERVER EMAIL_SMTP_PORT; do
    val=$(grep -E "^${key}=" "$PROJECT_DIR/.env" 2>/dev/null | head -1 | cut -d= -f2-)
...
<dict>
    ...
    <array>
        <string>$PY</string>
        <string>$PROJECT_DIR/worker.py</string>
    </array>
    ...
    <string>$PROJECT_DIR/worker.log</string>
...
if launchctl list | grep -q "$LABEL"; then
    ...
    echo
    echo "WARNING: worker did not come up. Check: tail -50 $PROJECT_DIR/worker.log"
    exit 1
fi
